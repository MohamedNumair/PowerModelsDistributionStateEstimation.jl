...
#     if cmp_bus == ref_bus
#         vat_block = (x, fc, br, td, d) -> vaf_ref[fc] - x[va_to[br]["$td"]]
...
#         end
#         vat_block = (x, fc, br, td, d) -> x[va_fr["$fc"]] - vat[br](x, br, td)[d]
#         if extra_block_needed  extra_block = (x, fc) -> x[va_fr["$fc"]] end
#     else
#         vat_block = (x, fc, br, td, d) -> x[va_fr["$fc"]] - x[va_to[br]["$td"]]
#         if extra_block_needed  extra_block = (x, fc) -> x[va_fr["$fc"]] end
#     end 
#     extra_block_needed ? (return vaf_block, vat_block, extra_block) : (return vaf_block, vat_block)
# end
...
#     vaf_block, vat_block = create_trigonometric_blocks(cmp_bus, ref_bus, t_buses, va_fr, va_to)
...
#                                   sum( -g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(vat_block(x, fc, br, td, d)) 
#                                        -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(vat_block(x, fc, br, td, d)) 
...
#                                         for (d, (fd, td)) in enumerate(zip(f_conn[br], t_conn[br])) if d!=c; init=0 )
#                                 -sum( -b[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*cos(vat_block(x, fc, br, td, d)) 
#                                         +g[br][c,d]*x[vm_fr["$fc"]]*x[vm_to[br]["$td"]]*sin(vat_block(x, fc, br, td, d)) 
#                                         for (d, (fd, td)) in enumerate(zip(f_conn[br], t_conn[br])))
#             )# closepush
#         end
#     end
...
#     vaf_block, vat_block, extra_block = create_trigonometric_blocks(cmp_bus, ref_bus, t_buses, va_fr, va_to, extra_block_needed = true)
