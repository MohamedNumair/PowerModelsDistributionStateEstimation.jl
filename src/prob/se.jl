...
"solves state estimation with angular reference relaxation by setting only one reference bus angle to zero"
function solve_angPha_mc_se(data::Union{Dict{String,<:Any},String}, model_type::Type, solver; kwargs...)
    if haskey(data["se_settings"], "criterion")
        _PMDSE.assign_unique_individual_criterion!(data)
    end
    if !haskey(data["se_settings"], "rescaler")
    ...
    return _PMD.solve_mc_model(data, model_type, solver, build_angPha_mc_se; kwargs...)
end
...
"NEW!! specification of the state estimation problem for a bus injection model - ACP and ACR formulations with Angular Reference Relaxation (Approach B only Ph.A = 0)"
function build_angPha_mc_se(pm::_PMD.AbstractUnbalancedPowerModel)

    # Variables
    #_PMDSE.variable_mc_bus_voltage(pm; bounded = true)

    _PMDSE.variable_mc_bus_voltage_angle(pm; bounded = true)                  #sets bounds for all terminals angles
    _PMDSE.variable_mc_bus_voltage_magnitude_only(pm; bounded = true)         # sets bounds for all terminals voltage magnitudes
    ...
    for (i,bus) in _PMD.ref(pm, :ref_buses)
        @assert bus["bus_type"] == 3      
        #constraint_mc_theta_ref_pha(pm, i)  #supposed to call the local version of the function in `constraint.jl`
        #display("$(bus["va"])")
    ...
    for (i,bus) in _PMD.ref(pm, :bus)
        ...
        # constraint_mc_voltage_bounds_se(pm, i) # there is variable_mc_bus_voltage
    end
